...
import core.scripting.ScriptStateFormat;
...
class ScriptState {
	...
	public static function loadScriptedState(path:String, ?args:Array<Dynamic>):MusicBeatState {
		if (path == null || !sys.FileSystem.exists(path)) {
			Trace.traceOnce('File not found: $path');
			return null;
		}

		sharedContext.types.set('ScriptStateFormat', ScriptStateFormat);

		var script = new RuleScript(null, null, sharedContext);
		script.errorHandler = (e) -> Trace.traceOnce('[ScriptState] $path → ${e.message}');
		script.tryExecute(sys.io.File.getContent(path));

		var className = haxe.io.Path.withoutExtension(haxe.io.Path.withoutDirectory(path));

		var access = RuleScript.resolveScriptedClass(className, sharedContext);
		if (access == null) {
			Trace.traceOnce('Not found the "$className" in $path');
			return null;
		}
	...
	public static function switchToScriptedState(path:String, ?args:Array<Dynamic>):Void {
		MusicBeatState.switchState(() -> loadScriptedState(path, args));
	}
